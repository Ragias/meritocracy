...
class User private () extends ProtoAuthUser[User] with ObjectIdPk[User] {
  ...
  object sex extends EnumField(this, Sex)
  ...
  object numCustomers extends LongField(this)

  object score extends LongField(this)
...
*/
  ...
  def profileScreenFields = new FieldContainer {
    def allFields = List(name, bio, location)
  }
...
object User extends User with ProtoAuthUserMeta[User] with Loggable {
  ...
  def whichUsername(in: String): List[String] = {
    User.where(_.username startsWith in).select(_.username).fetch(10)
  ...
  def getCurrentUserId = User.currentUser.get.id.defaultValueBox
  def findById(user_id: ObjectId): Box[User] = find(user_id)
